<template>
    <li role="treeitem" :class="classes" :draggable="draggable"
        ...
        @dragend.stop.prevent="onItemDragEnd($event, _self, _self.model)" @dragover.stop.prevent="isDragEnter = true"
        @dragenter.stop.prevent="isDragEnter = true" @dragleave.stop.prevent="isDragEnter = false"
        ...
        <ul role="group" ref="group" class="tree-children" v-if="isFolder" :style="groupStyle">
            <tree-item v-for="(child, index) in model[childrenFieldName]" :key="index" :data="child"
                :text-field-name="textFieldName" :value-field-name="valueFieldName"
                :children-field-name="childrenFieldName" :item-events="itemEvents" :whole-row="wholeRow"
                :show-checkbox="showCheckbox" :allow-transition="allowTransition" :height="height"
                :parent-item="model[childrenFieldName]" :draggable="draggable"
                :drag-over-background-color="dragOverBackgroundColor" :on-item-click="onItemClick"
                :on-item-toggle="onItemToggle" :on-item-drag-start="onItemDragStart" :on-item-drag-end="onItemDragEnd"
                :on-item-drop="onItemDrop" :klass="index === model[childrenFieldName].length - 1 ? 'tree-last' : ''"
                ref="childRef"
                >
                <template #default="_">
...
//vue3 向下查找需要的所有子组件，必须在onMounted之后执行
function findComponentsDownward(wantFind, context) {
    let list = []
    // dynamicChildren 是vue3的新增属性，可以获取当前组件下的所有子组件
    context.subTree.dynamicChildren.forEach(e => {
        if (e.type && e.type.__name === wantFind) {
            list.push(e)
        }
    })
    return list
        ...
        handleGroupMaxHeight() {
            if (!!this.allowTransition) {
                ...
                if (this.model.opened) {
                    let children = this.$refs.childRef
                ...
                if (this.$parent.$options.name === 'tree-item') {
