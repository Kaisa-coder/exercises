    ...
    <div :class="classes" role="tree" onselectstart="return false">
        <ul :class="containerClasses" role="group">
            <tree-item v-for="(child, index) in data" :key="index" :data="child" :text-field-name="textFieldName"
                ...
                :whole-row="wholeRow" :show-checkbox="showCheckbox" :allow-transition="allowTransition"
                :height="sizeHeight" :parent-item="data" :draggable="draggable"
                ...
                :on-item-drop="onItemDrop" :klass="index === data.length - 1 ? 'tree-last' : ''">
                <template #default="_">
...
export default {
    ...
    methods: {
        initializeData(items) {
            const key = this.childrenFieldName
            if (items && items.length > 0) {
                for (let i = 0; i < items.length; i++) {
                    const item = items[i];
                    var dataItem = this.initializeDataItem(item)
                    items[i] = dataItem
                    this.initializeData(dataItem[key])
                }
            }
        },
        ...
        handleRecursionNodeChilds(node, func) {
            if (func(node) !== false) {
                let children = node.$refs.childRef
...
<style lang="less" src="./less/style.less" />